/**
 * ファイル名: PasswordReset.tsx
 * 画面名: パスワード再設定画面
 * 説明: パスワード再設定機能を提供する画面
 * 機能:
 *   - メールアドレス入力
 *   - 確認コード入力
 *   - 新しいパスワード設定
 ...
 * パスワード再設定機能を提供します。
 *
 * @returns {JSX.Element | null} パスワード再設定画面コンポーネント。認証状態の復元中はnullを返します。
 */
export const PasswordReset: React.FC = () => {
  ...
  const { requestPasswordReset, confirmPasswordReset, isAuthenticated, isLoading: authLoading } = useAuth();
  ...
  const handleRequestSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStep('confirm');
      setMessage('パスワード再設定コードをメールアドレスに送信しました。コードを確認して入力してください。');
    } catch (err) {
  ...
  const handleConfirmSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      await confirmPasswordReset(email.trim(), confirmationCode.trim(), newPassword);
      setStep('complete');
      setMessage('パスワードの再設定が完了しました。新しいパスワードでログインしてください。');
      // 3秒後にログイン画面にリダイレクト
      setTimeout(() => {
        navigate('/login');
      }, 3000);
        ...
        {step === 'request' && (
          <form onSubmit={handleRequestSubmit}>
            <div style={{ marginBottom: '1.5rem' }}>
              ...
              />
              <p style={{ marginTop: '0.5rem', fontSize: fontSizes.small, color: '#6b7280' }}>
                登録済みのメールアドレスを入力してください。パスワード再設定コードを送信します。
              </p>
            </div>
          ...
          <form onSubmit={handleConfirmSubmit}>
            <div style={{ marginBottom: '1.5rem' }}>
              <p style={{ margin: '0 0 1rem 0', fontSize: fontSizes.medium, color: '#4b5563' }}>
                メールアドレスに送信された確認コードと新しいパスワードを入力してください。
              </p>
